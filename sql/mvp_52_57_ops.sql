...
-- mvp_115 (2026-09-08) core.data_source.alert : false 면 rows=0 일 때 상태가 '준비 중' (카드만 만들어 둔 원천).
--   fn_data_status 가 alert 을 같이 내려주고, 첫 데이터가 들어오면 rows>0 이 되어 저절로 정상/지연 판정으로 넘어간다.
--   tools/shoplinker/check-freshness.mjs : '준비 중' 은 실패(exit 1)·알림에서 제외하고 목록만 한 줄로 안내.
--   → GitHub Actions 가 sub_plan·web_inquiry 때문에 매번 빨갛게 실패하던 것 해결 (매장 일마감 지연 알림은 그대로 유지)
-- v67 화면 : 대시보드 채널 선택을 '집합'으로 — 범위 = 켜 놓은 채널(SEL). 전체/온라인만/오프라인만은 집합을 채우는 빠른 버튼일 뿐.
--   [채널 고르기] 팝업 : 온라인(고도몰·오픈마켓·직접B2B·홈쇼핑) / 매장 / 통신판매(VMS) / 렌탈 / 직배 3단 트리,
--   그룹·하위그룹 체크박스는 부분 선택이면 중간 상태, 각 줄에 해당 기간·지표 금액(그룹은 켠 금액/전체 금액), 즉시 반영,
--   빠른 버튼(전체·온라인만·오프라인만·모두 끄기·상위 5개), Esc·바깥 클릭으로 닫힘, sessionStorage 로 새로고침·기간 변경 후에도 유지.
--   필터 칩에 '범위 N/M개 채널', 채널 드롭다운 안내에 [켜고 끄기] 링크. 하나도 안 켜면 닫을 때 전체로 되돌림.
-- v68 화면 : 홈페이지 문의 · 구독 상품도 다른 원천과 같은 방식으로 — 데이터 소스 카드에 [파일 올리기][붙여넣기][양식] 표준 버튼.
...
-- v74 화면 : 주문서 표 머리글 정렬이 서버 정렬로 연결됨(주문일·매출·순매출) — 지금까지는 보이는 50행만 뒤집어서
--            "오래된순" 으로 눌러도 첫 화면이 그대로였음. 이제 정렬 칩(최신순/오래된순/금액순)과 같이 움직이고 1쪽부터 다시 부름.
--            주문서 페이저에 « 처음 · 끝 » · 쪽 번호 입력(Enter) 추가 (27,000건이면 541쪽이라 이전/다음만으로는 못 감)
-- v75 화면 : 페이징 통일 — 모든 목록이 [10/20/50/100행 선택 + 숫자 쪽 버튼(« ‹ 1 2 3 … ›  »)] 로 바뀜. 기본 10행.
--            선택한 행 수는 목록별로 브라우저(localStorage 'dc_pg')에 기억. 공통 함수 pgSize()/pgBar()/pgSet().
--            관리자 : 입출고·품목·통합원장·문의접수·홈페이지문의·상담·고객·CRM대상·서비스로그·주문서대기열
--            담당자 : 재고 조회 · 주문 변환 목록 · 전체 상담·배정

-- ============================================================
-- mvp_121 : 배정 규칙에 '관심 품목' 추가 + 담당자 화면 구독 제품번호 검색
-- ============================================================
--   core.f_assign_next(p_scope, p_item) : 후보 scope 를 'item:<관심품목>' → <접수 종류> → 'default' 순으로 훑어
--     담당자가 있는 첫 규칙에서 순번을 돌린다. (1인자 버전은 drop, 기본값으로 대체)
--   fn_submit_customer : product_name(폼에서 고른 관심 제품)을 p_item 으로 넘김
--   fn_submit_visit    : 지금까지 담당자를 아예 안 넣었음 → interests[1] 로 배정해 handler 채움
--   fn_assign_pool()   : items 목록 추가 (폼 고정 목록 + 실제 접수된 값 + 이미 만든 item 규칙)
--   fn_store_sub_models(p_code, p_q, p_limit) : sub.plan 의 제품번호 574개를 담당자 화면에서 검색
...
-- v80 화면 : 담당자 홈에서 [전체 상담·배정] 카드를 입력 버튼 바로 아래(맨 위)로 올림.
--            목록 각 줄에 [상담 이동] 버튼 추가 — 이름이 없는 건(이카운트 가망 등)도 상담 입력으로 넘어갈 수 있게.
--            이름이 있으면 이름 클릭도 같은 동작. 이름·연락처가 없으면 안내 토스트 후 이름 칸에 커서.

-- ============================================================
-- mvp_124 : 담당자 화면 개편(사이드바·모바일) · 새 문의 알림 · 관리자 첫 화면 속도
-- ============================================================
-- [새 문의 알림] fn_store_requests 에 inbox 추가 — 내게 배정된 진행중 건 수 · (점장·대표는) 미배정 수 · 마지막 id · 마지막 건 요약
--   화면은 20초마다 보고 last_id 가 커지면 : 토스트 + 진동 + 브라우저 알림(권한 있을 때, 탭이 뒤에 있어도) + 홈 목록 갱신.
--   탭을 다시 보면(visibilitychange) 즉시 한 번 더 조회. 알림 권한은 PIN 입력 직후 한 번 묻는다.
--   즉시 푸시(화면이 닫혀 있어도)는 아직 아님 — 잔디 웹훅 / Web Push 는 다음 단계.
-- v81 화면
--   담당자 : 왼쪽 사이드바(≥1000px, 내 업무/매장/현황 3묶음) · 모바일은 아래 탭바 4개 + 더보기 서랍. 기존 .tabs 는 숨기고 거울로 그림(syncNav)
--            홈 탭에 배정 건수 배지, 주문서 탭에 요청 배지. 전체 상담 기본 필터를 '진행중 전체'로(자동 배정 뒤로 미배정이 드묾)
--            폼 : 필수 표시를 * → [필수] 빨간 배지로, 필수 칸은 채우기 전 살구색, 필수 칩 묶음은 안 고르면 점선 살구색, 고를 수 있는 칩은 연남색·고른 칩은 남색
--   관리자 : 엑셀 모듈(900KB)을 첫 화면에서 빼고 쓸 때 불러옴(내보내기·올리기·양식) · 프로필/메뉴/담당자 3번 왕복 → 동시 요청 · supabase/cdn preconnect
