...
-- 2026-09-22 후속 ("혼수/입주/이사 각각이 포함될 때만 그렇게"): 규칙은 처음 판 그대로 — 그 유형이 포함돼 있으면 그 유형의 날짜 필수(혼수 결혼 예정일 · 입주 입주일 · 이사 이사일),
--   셋 중 하나라도 있으면 증빙 여부 필수(추후면 날짜), 혼수만 희망 배송일 필수. 잠깐 '혼수만 필수'로 풀었다가 되돌렸다(`v_proof is distinct from '추후'` 만 남음).
--   폰에서 본 "결혼 예정일(혼수)을 입력하세요" 는 서버 문구 — 날짜 칸이 없는 옛 화면(v147 캐시)이 새 유형 코드를 보내 서버에서 막힌 것. 화면은 이제 검증 토스트마다 그 칸으로 스크롤·포커스(`cGo`), 서버 거절도 문구로 칸을 찾아간다(`cGoByMsg`).
--   주의: 함수 패치 do 블록과 raise 로 끝나는 롤백 테스트를 같은 execute_sql 에 넣으면 패치까지 롤백된다 — 이번에도 한 번 당해 다시 적용했다.
