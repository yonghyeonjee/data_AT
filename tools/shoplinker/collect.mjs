...
const DAILY_JOBS = [
  ...
  { name: "취소/교환/반품", flag: "999", dateType: "001", days: 90 },   // 2026-09-21: 30→90일. 취소완료(013)·반품완료(008)는 주문 뒤 몇 주 지나 찍힌다
];
// 2026-09-21 --flags 탐색 결과 (샵링커 API 의 한계 — 바꿀 수 없는 것):
//  · 받는 코드는 002·003·015·999 뿐. 001·004~014·016~020 은 "가능한 범위코드 : 002,003,015,999" 로 거부. '신규주문' 코드는 없다.
//  · 002 는 발주확인 '대상'(발주일 빈 주문)도 돌려준다. 주말 주문은 샵링커가 월요일 아침에 몰에서 가져오므로 그 전엔 어느 코드로도 안 보인다.
//  · 999 는 취소완료(013)·반품완료(008)만 준다 — 60일치 5,500건 중 22건. 배송전취소·주문취소요청·반품요청 같은 중간 상태는 API 에 없다.
//    옛 원장 시트(샵링커 웹 내려받기)엔 월 100~260건의 취소가 있었다 → 취소·환불을 제대로 보려면 샵링커 웹 주문목록(취소/반품 상태) 내려받기를 올려야 한다.
