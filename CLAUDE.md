...
- **고도몰 과거 주문 → 통합 원장 (mvp_180 · v151 · 2026-09-22)** — 요청서 "raw.godo_order_hist 의 2021~2025 4몰 주문을 core.orders 로, 샵링커와 안 겹치게". `core.f_godo_hist_apply(p_dry, p_include_unmatched_after, p_by)` · 관리자 래퍼 `fn_godo_hist_apply`. **source = `godo_hist`**(check 제약에 추가), channel_type 자사몰, channel_name P몰·S몰·AT몰·시흥몰, channel_account pcnik35-P/S/AT·samsungsh(샵링커 행과 같은 값).
  ...
  **줄 금액 규칙(요청서와 다른 점)** — 엑셀 `판매가` 에는 옵션 추가금(+75,000 ×4 = 30만)이 빠져 있고 `상품수량` 이 1 로 찍힌 다량 주문(67,500 × 36 = 2,430,000)도 있어 `판매가×수량` 은 22,344 주문 중 2,205건에서 주문 총액과 어긋난다. 그래서 **주문의 `총 품목 금액`(API `총상품금액`)을 (판매가+옵션 추가금)×수량 비율로 줄에 배분**(반올림 잔차는 마지막 줄) — 19,909 주문 전부 sum(gross) = 총 품목 금액. 주문 단위 값(총 결제·총 할인·총 환불)은 줄마다 더하지 않는다. unit_price = 판매가+옵션 추가금, discount = 줄의 쿠폰·회원·앱 할인, shipping_fee = 총 배송 금액을 첫 줄에만.
  취소·환불 줄(환불완료·환불접수·환불보류·관리자취소·고객요청취소·반품접수 / API c·b·r 계열)은 refund = gross, cancelled_at = 클레임 완료일자/취소일. 교환·배송완료·구매확정은 매출. `net_amount`·`model_key`·`category`·`goods_kind` 는 **generated 열**이라 insert 에 못 넣는다(54023 아님 428C9). `source_ref` 는 (source, source_ref) unique 라 `몰:상품주문번호`(겹치면 :주문번호:줄).
  **API 상태·결제수단 코드표는 `core.godo_code`** — 고도몰 문서를 컨테이너에서 못 열어(egress 차단) godomall5 매뉴얼 기억으로 넣었다(o1 입금대기 · p1 결제완료 · g1 상품준비 · d1 배송중 · d2 배송완료 · s1 구매확정 · c1~c4 취소 · b1~b4 반품 · e1~e5 교환 · r1~r3 환불 · f1~f3 결제 안 됨 / 결제수단 gb·pb·pc·ph·pv·eb·em·ev, f* 는 '간편결제(fx)' 로 표시). **틀리면 표만 고치고 `f_godo_hist_apply(false)` 를 다시 돌리면 된다**(sale=false 면 제외, refund=true 면 환불).
  **주문번호가 몰 사이에 겹치는 8건**(다른 고객·다른 주문: 2107301133000001 AT몰/S몰 …)은 두 번째 몰의 order_no 에 `#몰` 을 붙였다(unique (source, order_no, line_no)). `모델명` 이 '상세설명참조' 류(5,190줄)면 null 로 두어 model_key 가 상품명에서 모델을 뽑는다. buyer_key = `core.f_buyer_key(이름, 휴대폰)`(이메일 폴백 없음, 95줄 키 없음) — **crm.customer 는 건드리지 않았다**(요청서 3).
  결과: **21,295줄 · 19,909주문 · upload 534**. 몰×연도 순매출(백만): AT 2021 69 · 22 70 · 23 210 · 24 1,190 · 25 451 / P 2022 88 · 23 542 · 24 1,550 · 25 767 / S 2021 1,558 · 22 1,939 · 23 1,187 · 24 719 · 25 279 / 시흥 2024 27 · 25 306. 마지막 order_at 2025-06-24(각 몰 샵링커 시작 전날까지) → **2026 수치 변화 없음**(shoplinker 8,881.5백만/24,509줄 그대로). `f_dash_payload` 원장 모드 자사몰 출처 목록 2군데에 godo_hist 추가(캐시 비움) — 24개월 창이라 /dash/ 에는 2024-09 이후분만 보인다.
  화면: 통합 원장 출처 select 에 **[고도몰 과거]**(`godo_hist`). 성능: 처음 판은 상관 서브쿼리(충돌·겹침·시작일) 때문에 60초를 넘겼다 → 미리 묶은 CTE 조인으로. **실행 중 MCP 가 60초에 끊겨도 서버는 끝까지 돌 수 있다** — 재적재 때 upload 534 가 그렇게 생겼다(끊긴 뒤 상태를 확인할 것).
  **2026-10-10 주문(요청서 4)** = `rental` 렌탈 월 청구 줄(주문번호 '2026/09/10 -6' 등) — 렌탈은 청구시작일 귀속이라 10월 청구가 미리 잡혀 있다. 오류 아님.
- **통합 원장 머리글 정렬은 검색된 전체를 서버에서 (v151)** — "모든 필터는 검색된 전체 영역에서 작동하게 … 10페이지에 있던 내림차순이면 오름차순일때 1페이지로". 머리글 클릭이 주문일·순매출 두 열만 서버 정렬이었고 나머지는 아무 일도 안 했다.
  `fn_order_list.p_sort` 를 `열_asc|desc`(date·amt(순매출)·gross(매출)·refund·qty·channel·kind(구분)·customer·product·status·handler) 로 넓히고(OUT 열이 있어 `pg_get_functiondef` 통째 치환), `SRV_SORT.tbOrders.cols` 에 11개 열을 매핑, `firstDesc`(주문일·수량·매출·환불·순매출은 첫 클릭 내림차순). 누르면 `#oSort` 값이 바뀌고 `loadOrders(0)` = 1쪽부터. `#oSort` 에 같은 값 옵션(optgroup) 을 두어 select 로도 고를 수 있다. 테스트 `ptest/osort.mjs` S1~S3 · ovf_admin 전부 ✓.
- **상담 입력 검증 — 빠진 칸으로 데려간다 (v150 · 2026-09-22)** — 폰 사진에 "결혼 예정일(혼수)을 입력하세요" 토스트만 뜨고 어디를 채워야 할지 안 보였다. 그 문구는 **서버** 것(`%(%)을 입력하세요`) — 날짜 칸이 없는 옛 화면(v147 캐시)이 새 유형 코드를 보내 서버에서 막힌 경우. "alert 띄울 때 그 칸으로 강제 이동".
  `cGo(id,msg)` = 토스트 + `scrollIntoView(center)` + 1.6초 빨간 테두리(`.need`) + 포커스(칸이 div 면 안의 첫 input·button). `consultCollect` 의 모든 검증이 이걸 쓰고, 서버가 거절하면 `cGoByMsg(e.message)` 가 문구(결혼 예정일·입주일·이사일·희망 배송일·증빙 가능 날짜·증빙 여부·고객명·문의 유형·관심 품목·담당자)로 칸을 찾아간다.
  **필수 규칙 확정** ("혼수/입주/이사 각각이 포함될 때만"): 포함된 유형의 날짜(혼수 결혼 예정일 · 입주 입주일 · 이사 이사일) · 셋 중 하나라도 있으면 증빙 여부(추후면 날짜) · 혼수만 희망 배송일. 잠깐 '혼수만 필수'로 풀었다가 되돌렸다 — 화면·서버·여러 건 표 셋이 같은 순서로 검사한다.
  **함수 패치 do 블록과 raise 로 끝나는 롤백 테스트를 한 execute_sql 에 넣으면 패치까지 롤백된다** — 이번에도 당했다(문서에 이미 있었는데). 반드시 두 호출로. 테스트 `ptest/tyx.mjs` T2 에 스크롤·포커스·강조 검사, T3·T5 유형별 필수 (16/16), UAT 4조합·grid·tour 폰 13탭 ✓.
...
- **상담 입력 문의 유형 — 여러 개 · 혼수/입주/이사 (mvp_178 · v148 · 2026-09-22)** — "실제 담당자들은 일반 상품(일시불), 구독, 혼수, 입주, 이사에 따라 다르게 상담을 진행함". `core.inq_type` 에서 **일시불 문의(onetime) 끔**(상품 문의로 충분), **혼수(wedding)·입주(movein)·이사(moving)** 추가 — 표에 `date_label`(결혼 예정일·입주일·이사일)·`needs_proof` 열이 있어 화면은 `fn_inq_codes` 의 `types[].date_label/proof` 만 보고 칸을 만든다(유형을 더 넣을 때도 이 표 한 줄).
  ...
  **[＋ 날짜 추가]**(`xdAdd`, 이름 + 날짜) — DB 에 열을 만들지 않고 `extra_dates` 로 보내면 서버가 notes 에 '추가 날짜 · 예식장 계약일 2026-09-30' 로 남긴다(상세 팝업 진행 기록에 보임). 여러 건 표 `GCOLS.c` 에 행사일·희망 배송일·증빙 여부·증빙 가능일 칸(유형은 줄마다 하나). 상세 팝업 [어떻게 들어온 문의인지] 에 결혼 예정일·희망 배송일·증빙 줄. `crm.consult_scoped` 뷰에 새 열 덧붙임.
  **판매 쪽은 안 바꿨다** — 판매 입력의 구분(일시불·구독·직판)은 그대로고 상담→[구매 확정]·구매함 저장(`consultSend`)도 그대로. 혼수·입주·이사는 상담(`crm.consult`)에만 있고 판매는 `consult_ref` 로 상담에 이어지므로 나중에 '혼수 판매' 를 세려면 상담을 거쳐 본다.
  `fn_store_consult_detail` 은 `jsonb_build_object` **인자 100개 한계**에 걸려(54023) 새 키를 `|| jsonb_build_object(...)` 로 뒤에 붙였다 — 이 함수에 키를 더 넣을 때도 그렇게. 테스트 `ptest/tyx.mjs` T1~T6 × PC·폰 = 14, grid C1 칸 수 26 → 30, UAT 4조합·tour·audit 통과. 안내서 '어떻게 들어온 문의인지' 한 줄 추가.
  **다음 단계(아직)**: 품목 줄마다 일시불/구독·모델·예상 금액(`crm.consult_item`) → 일시불·패키지 견적서 양식 · 옛 관심 품목 별칭.
- **대시보드 `/dash/` 발표 흐름 — 몰 머리 바 (v147 · 2026-09-22)** — "채널에서 상세 넘어가서 발표하기 불편한것 같은데 … 사용자 입장을 고려한 UI/UX". 채널 화면과 상세 화면 맨 위에 **같은 몰 머리 바**(`mallHead(ci,mode)` · `bindMallHead` · 상세는 `#dv-head`/`drawDvHead`, `.cvsticky` 라 스크롤해도 위에 남음):
  몰 이름 · 전기 대비(`mallDelta`) · **◀ n/N ▶**(`cv-prev/next` · `dv-prev/next`, ← → 키는 채널·상세 둘 다) · **[요약 | 상세]** 세그(`.mseg [data-mv]`, 같은 몰로 화면만 바꿈) · **[✕ 몰 해제]**(`.mclr`). 채널 화면의 칩 카드는 더 이상 sticky 가 아니다.
  같이 고친 것: ① **채널 화면에서 채널을 지우면 개요로** — 필터 칩 ✕ · 몰 선택 '전체 채널 보기' · [전체 해제] · 채널 칩 다시 누르기 · Esc 전부 `clearFocus()` → `view==='c'` 면 `setView('o')`. ② 서랍(개요에서 몰 열기) 폭 `min(880px,96vw)` + 맨 위 [이 채널 전체 화면으로 보기 →] 한 줄 전체·15px(`.dw-fullbtn`).
  ③ '환불 데이터가 없습니다 …' 멘트를 서랍·채널 화면·팩트에서 뺐다(환불율 칸의 '환불 미기록' 태그는 그대로). ④ **서랍에서 연 몰이 위 몰 선택(`#mallsel`)에 안 따라오던 버그** — `openCh` 가 `drawMallSel()` 을 안 불렀다(테스트 M12b 가 잡음).
  **dash 내부(`render`·`focus`·`setView`…)는 클로저라 테스트에서 못 부른다** — `ptest/dashmall.mjs` M11~M13 은 DOM 만 누른다(화면 = `#v-*[aria-pressed]`, 초점 = `#mallsel.value`, 칩 = `#abar [data-x="focus"]`). 38/38 × PC·폰, dashpres 20 · dashqty 3 · dashcat 16 · dashmode 3.
- **VMS 에 냉장고가 보이던 것 = 이카운트 '오픈마켓' 전표 (mvp_177 · v146 · 2026-09-22)** — "이카운트에서 VMS 인 것만 가져온 것 맞지?" → **아니었다.** 판매현황 업로드는 VMS 필터 없이 파일 전체를 channel VMS 로 넣는다. 2020~2026-08 초까지는 영업담당이 택배·퀵·이정우·오영환·고병재… 라 전부 소모품 통신판매였는데,
  9/18 `판매현황_0918.xlsx` 에 **영업담당 '오픈마켓' 전표(2026-08-31 자 온라인 몰 월 마감, 2,561줄 · 7.15억 — 프라자몰·SSG·스마트스토어 B2B·쿠팡·에스몰·시흥몰…)** 가 섞여 있었다. 샵링커 원장에 이미 있는 매출이라 **이중 계상**이고 냉장고·세탁기·TV 가 VMS 에 보인 것도 이 줄들.
  정리: notes `영업:오픈마켓%` 2,586줄(7.17억, 옛 25줄 포함)을 `core.orders_ecount_openmarket_20260922` 에 두고 삭제, `dash_cache` 비움. 남은 VMS 대물은 박은지 B2B 5줄(케이모텔 세탁기 등)로 정상.
  재발 방지: `core.f_orders_bulk_upsert` 가 ecount 줄의 notes 가 '영업:오픈마켓' 이면 건너뛴다(응답 `오픈마켓제외`), 화면 `mapRows` 도 빼고 미리보기에 "오픈마켓 전표 N줄은 제외"(`ecountGuard`). **이카운트 판매현황 = 영업담당이 오픈마켓이 아닌 줄만 VMS.** 이카운트 쪽에서 온라인 전표를 다른 방식으로 적기 시작하면 이 규칙도 같이 봐야 한다.
...
- **관심 품목은 견적서 품목표와 같은 24개 (v136 · 2026-09-21)** — "혼수/입주/이사 vs 구독 vs 일시불마다 상담 입력과 견적서가 달라져야 한다" 논의의 첫 조각. 담당자 화면 `CATS` 가
  TV·사운드바·벽걸이·후드·냉동고·냉장고·김치냉장고·페어킷·세탁기·건조기·에어드레서·슈드레서·에어컨·식기세척기·인덕션·오븐(큐커)·공기청정기·로봇청소기(구독)·무선청소기 제트·무빙스타일·정수기(구독)·밥솥·제습기·MX(모바일)·노트북(v137)·기타·알 수 없음 (옛 11개 대분류 `세탁·건조`·`주방가전`·`입주패키지` 등은 뺐다 — 옛 상담의 값은 그대로 남아 통계에 그 이름으로 보인다).
  상담 기록 `#c_cat` 은 정적 HTML 이 아니라 `catChipsHtml()` 로 그리고, 여러 건 표(`GCOLS.c` CAT)·문의 접수(`hoRenderCat`)도 같은 함수. 라벨 `catLabel`: `기타` → "기타 (직접 입력)"(값은 `기타`, 적은 말로 저장하는 규칙 그대로). `알 수 없음` 은 필수 검사 때문에 마지막에 남겼다.
  **다음 단계(논의 중)**: ~~계기(혼수·입주·이사·교체) 칸 + 예식일/입주일~~(mvp_178 로 문의 유형 여러 개 + 날짜로 구현) → 품목 줄마다 일시불/구독·모델·예상 금액(`crm.consult_item`) → 일시불·패키지 견적서 양식(엑셀 `견적서-일반+구독` 왼쪽 혜택 계단 + 오른쪽 구독 계산표) → 카드 요율표 DB 화. 24개 목록도 언젠가 `core.item_cat` 표로 뺀다.
...
- **대시보드 `/dash/` 몰별 발표 개선 (v141 · 2026-09-22)** — "각 몰 담당자가 발표할 때 이 과정이 더 수월하면 · UI 도 너무 다 펼치지 말고".
  ① **프리셋 [올해]·[작년]**(`ty`·`ly`) — 올해 = 1/1~마지막 날, 비교는 전년 같은 기간(`cmpRange`), 작년 = 1/1~12/31, 비교는 전전년(자료 없으면 없음). 작년 자료가 아예 없으면 버튼이 잠긴다.
  ② **채널 화면 표 단위는 선택 기간의 한 단계 아래** — 7일 이하 일별 · 45일 이하 주별 · 그 위 월별(`sg`). 한 달을 골랐는데 "월별 9월 한 줄" 이던 것.
  ③ **KPI 를 몰 발표용 8칸으로** — 순매출·주문 건수·**판매 개수(N개·M종)**·구매 고객·**신규 고객**·재구매·객단가·전체 비중, 칸마다 **전기 대비 작은 글씨**(`.kpi .sub`). 총매출·환불·환불율·일평균은 [세부 지표] 접는 카드로.
  ④ **카테고리를 누르면 오른쪽 패널**(`#cvright`)에 그 카테고리 상위 상품 — 아래로 밀지 않는다. 기본은 증감 원인(상품), [✕ 증감 원인으로] 로 복귀, 몰을 넘기면 자동 복귀. `cvCatOpen`(하나만) · `drawCvRight(ci,pa)`. 개요의 카테고리 펼침(`CAT_OPEN`, 아래로)은 그대로.
  ⑤ **몰 이동 [◀ 1/17 ▶]**(`.mnav`, `cvList`·`cvStep`) + ← → 키(채널 화면 · 발표 모드 아님 · 서랍 닫혀 있을 때). 순서는 칩과 같다(범위 안 · 매출순). 위 몰 select 도 같이 바뀐다.
  ⑥ **접는 카드**(`.card.fold[data-fk]`, `bindFolds`, 상태는 `sessionStorage dash_fold`) — 채널 화면 [세부 지표]·[일/주/월별 표]·[이 채널 상위 상품]·[기간 내 이벤트], 상세 [월 × 채널 매트릭스]는 **기본 접힘**. 머리를 누르면 펼쳐지고 `align-self:start` 라 옆 카드 높이만큼 안 늘어난다. 머리 40px↑.
  ⑦ **상세 화면에 몰 필터가 걸려 있으면** 채널별 성적표 → **그 몰의 카테고리별 성적표**(선택 기간·전기 대비·비중·판매 개수, 줄을 누르면 상품 목록이 그 카테고리로), 월 × 채널 → **월 × 카테고리**, 기간 팩트 → 1위 카테고리·상위 3 집중도·구매 고객·최대 증가/감소 **상품**(`mallCats`). 해제하면 원래대로.
  테스트 `ptest/dashmall.mjs` M1~M10 × PC·폰 = 30. dashpres P6 는 오른쪽 패널로, dashqty Q2 는 접힌 카드를 펼치고 누르도록 고쳤다(20/20 · 3/3). dashcat 16 · dashmode 3 그대로.
  **자료 확인(2026-09-22)**: 캐시 12:15 갱신 · 샵링커 09-21 · 이카운트 09-22 까지 반영. **[작년](2025)은 자사몰이 6/20 부터만 있다** — 고도몰 과거 주문(raw)을 원장에 넣기 전엔 2025 상반기 자사몰이 비어 전년 비교가 낮게 나온다(⑥ 판단 대기).
- **관리자 통합 원장 3건 (v141 · 2026-09-22)** — ① **주문일이 하루 앞으로 보이던 것**: `dfmt` 가 timestamptz(UTC ISO)를 앞 10자만 잘라 자정~09시 주문이 전날로 찍혔다(올해를 골랐는데 12/31). 서버 필터(`fn_order_list`)는 KST 라 화면만 `toLocaleDateString('sv-SE',{timeZone:'Asia/Seoul'})` 로 맞췄다. 날짜만 있는 값은 그대로.
  ② 기간 바에 **[작년]**(`ly`, `setPreset`·`PRESET_NM`·`cmpRange` 전전년). ③ 출처 select `샵링커(온라인)` → **`온라인`**(값은 `shoplinker` 그대로 — 고도몰 과거 주문이 원장에 들어오면 그때 묶는다).
- **데이터 소스 카드를 수동 / 자동으로 나눴다 (mvp_173 · v140 · 2026-09-22)** — "카드들이 잘보이게 요약 정보로 하고, 수동, 자동 나눠두고".
  `core.data_source.feed_mode`(`auto`·`manual`·`mixed`) → `fn_data_status` 응답에 실리고, 화면이 **✋ 수동 업로드 → ◐ 수동 + 자동 → ⚙️ 자동 수집** 순서로 묶는다.
  묶음 머리(`.dsgrp`)에 이름 · 개수 · **[확인 필요 N]**(그 묶음의 비정상 수) · 한 줄 설명. 손이 가는 것과 알아서 들어오는 것이 섞여 있어 뭘 올려야 하는지 안 보이던 것을 고쳤다.
  **고도몰 주문 카드는 몰별 요약**(`dsDetail`) — AT·P·S·시흥 각각 줄 수 · 주문 수 · 기간 · 수동/자동 태그. `fn_data_status` 에 `godo_detail` CTE 추가.
  **과거 자료 보관함은 경과일을 빨갛게 하지 않는다** — `ARCHIVE={godo_order:1}` 이면 `30일` → `30일 적재`(loaded_at 기준), 경과 `387일 (과거 자료)` 회색. 2025-08 이 마지막인 게 정상이라서다.
  `fn_data_status` 의 stat CTE 에 `raw.godo_order_hist` 를 넣고(전엔 행 0 · 준비 중), `up` CTE 에 `godo_order_hist` → `godo_order` 매핑을 더해 마지막 적재·파일 수가 뜬다. 테스트 `ptest/dsgroup.mjs` D1~D6 × PC·폰 = 16.
- **고도몰 주문 원본 적재 완료 (2026-09-22)** — `raw.godo_order_hist` **26,962줄 · 주문 25,216건**.
  | 몰 | 방식 | 줄 | 주문 | 기간 | 유효 | 유효 금액 |
  |---|---|---|---|---|---|---|
  | S몰 | 파일 | 11,999 | 11,132 | 2021-01-01 ~ 2025-08-31 | 8,623 | 52.1억 |
  | P몰 | 파일 | 7,015 | 6,884 | 2021-04-12 ~ 2025-08-31 | 5,746 | 29.9억 |
  | AT몰 | 파일 | 5,067 | 4,328 | 2021-01-01 ~ 2025-08-30 | 3,735 | 20.6억 |
  | 시흥몰 | OpenAPI | 2,881 | 2,872 | 2024-10-14 ~ 2025-08-31 | 938 | 5.2억 |
  **빠진 구간은 P몰 2021-05 ~ 2021-12 여덟 달뿐**(2021-04 에 2줄, 2022-01 부터 정상). 나머지 세 몰은 첫 달부터 끝까지 달마다 자료가 있다. 네 몰 모두 **2025-08-31 이 마지막** — 그 뒤는 샵링커가 받는다.
  **P·AT·S 파일은 원본 97열 이름이 그대로 들어 있다** (`주문 번호`·`주문자 핸드폰 번호`·`총 결제 금액`·`수취인 나머지 주소`·PG·마일리지까지). mvp_172 화면 매핑(29칸 정규화)을 거치지 않은 경로로 올라왔다는 뜻이다.
  시흥몰(API)만 정규화 이름(`주문번호`·`주문자휴대폰`·`결제금액`)이라 **두 벌이 섞여 있다** — 집계할 때 `coalesce(data->>결제금액, data->>총 결제 금액)` 식으로 둘 다 봐야 한다. 원장에 넣기 전에 한 벌로 정리할 것.
  상태 표기도 다르다: 파일은 한글(구매확정·환불완료·결제실패·고객결제중단), API 는 코드(s1·d1·f1·r3).
  샵링커와 겹치는 구간(2025-06-20 ~ 08-31)은 고도몰 1,962줄 vs 샵링커 자사몰 1,122줄 — **중복 경계를 정해야 원장에 넣을 수 있다**(⑥ 판단 대기).
- **P·AT·S몰 주문은 파일로 올린다 (mvp_172 · v139 · 2026-09-22)** — exe 대신 **관리자 › 데이터 가져오기** 에서 받는다. 고도몰 관리자 주문 내려받기(97열)를 그대로 올리면 된다.
  인식은 열 **`자체상품코드`**(이 양식에만 있다) → `godo_order`. **몰(P·S·AT·시흥)을 고르기 전에는 [적재] 가 잠긴다.** 미리보기는 저장될 칸을 그대로 보여준다.
  **열 이름은 시흥몰 OpenAPI 와 똑같이 맞춘다** — 주문번호·주문일자·결제일자·주문상태·결제금액·총상품금액·배송비·결제수단·환불금액·회원ID·회원명·주문자명·주문자휴대폰·주문자전화·이메일·배송지역·상품번호·상품코드·모델명·상품명·옵션·수량·상품가격·정가·카테고리·상품주문번호·품목상태·취소일·줄번호. 그래야 원장 반영을 한 번만 짠다.
  **버리는 것**: 예치금·마일리지 전부 · PG · 입금계좌·입금자·영수증 · 배송업체·송장 · 사은품 · 수수료율·매입가 · 브랜드·제조사·원산지 · 수취인 이름/전화/우편번호/**상세주소** · 주문시 남기는 글 · 환불 세부 · 클레임 사유·수량 · 모바일앱 할인.
  **주소는 시·군·구까지만**(`배송지역` "경기도 김포시") — 상세주소는 CRM 에 쓰지 않으므로 아예 저장하지 않는다. `0원` 같은 글자는 숫자로 정리하고, **한 주문에 상품이 여러 줄이면 주문 칸이 비어 있어도 직전 주문을 물려받는다**.
  **해시는 서버가 만든다 (mvp_172)** — 브라우저는 sha1 을 못 만들어 `h` 없이 보내고 `core.f_godo_order_hist_put` 이 `sha1(몰|주문번호|상품번호|줄번호)` 로 채운다. exe 규격(h 직접 전달)은 그대로 살아 있다. 같은 파일을 다시 올려도 중복되지 않는다.
  주문상태는 엑셀이 한글(구매확정·배송완료), API 는 코드(s1·d1) — **원장 반영 때 대응표가 필요하다**(⑥ 판단 대기에 포함). 테스트 `ptest/godoorder.mjs` G1~GD (14/14).
- **시흥몰 OpenAPI 수집 완료 · 페이징 버그 (2026-09-22)** — `raw.godo_order_hist` 에 **2,881행 · 주문 2,872건 · 2024-10-14~2025-08-31**.
  **고도몰은 `size`(100)를 채워 주지 않는다** — 첫 판 `collect.mjs` 가 `batch.length < PAGE` 로 끊어 조각마다 첫 페이지만 받았다(3/1~3/30 이 81건에서 멈춰 **3/1~3/20 이 통째로 누락**). 9/21 1차 수집 601행이 그것.
  콜랩 원본대로 **빈 페이지가 나올 때까지 `lastOrder` 커서를 밀고** 상한은 `MAX_PAGES`(60)로만 막는다 → 조각당 p1~p4, 601 → **2,881행**. 콜랩이 2025-01~08 에서 받은 1,065건도 같은 이유로 적게 받은 것이다(우리 2025년 2,586행).
  **2021~2024-10-11 은 주문이 0건** — 조각을 다 돌았는데 빈 응답이다(로그에 p1 줄조차 없다). 시흥몰 자사몰이 2024-10 부터라는 뜻이고, 요청서의 "2021~2025.07" 은 시흥몰에 한해 **2024-10 이후만 존재**한다.
  상태 분포: f1(결제 실패) 1,937 · s1(입금완료) 632 · r3 165 · d1(배송완료) 120 · f2 10 · 나머지 17. **f1 은 결제 시도만 하고 안 된 건이라 매출이 아니다**(금액 31억은 장바구니 금액). 유효(f1·f3 제외) 938행 · 5.19억 · 휴대폰 939개.
  개인정보는 f1 을 빼면 100% 채워져 있다(이름·휴대폰). f1 은 1,937건 중 134건만 — 결제 전에 빠져나간 것.
  **원장·고객에는 아직 안 쓴다**(⑥ 판단 대기 그대로): 채널 매핑 · 2025-06-20 이후 샵링커 자사몰 주문과의 중복 경계 · f1 처리 · 휴대폰 단독 키 합치기.
  P몰·AT몰·S몰은 OpenAPI 키가 없어 여전히 exe(`tools/godo/GodomallCollector_spec.md`) 대기.
