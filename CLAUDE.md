...
- **상담을 [연락 전]으로 되돌린다 (mvp_159 · v118 · 2026-09-18)** — 이수혁 프로가 안현지 건에 콜백(09-19 14:00)을 잡자 `fn_store_consult_update` 의 callback 이 `진행전`→`진행중` 으로 올려 버렸고, 상태 바의 [연락 전]은 `act:null` 이라 되돌릴 길이 없었다("연락전으로 돌리려면 어떻게 해").
  ① `p_action='pre'` 추가 — result=`진행전`, notes '연락 전으로 되돌림 MM-DD HH:MI 이름', **콜백 약속은 그대로**. ② callback 의 승격 case 에서 `진행전` 을 뺐다 — **연락 전에 콜백만 잡으면 연락 전 그대로**(종료·거절→진행중은 유지).
  화면 `STATE_STEPS[진행전].act='pre'`(버튼 켜짐, `msg` 로 토스트 문구), 힌트에 되돌리기 안내. 안내서 상태 줄·상태 표에도 한 줄. id 390(안현지)은 손으로 진행전으로 돌렸다. 확인은 `ptest/pre_probe.mjs`(데스크톱·폰, 버튼 활성·p_action pre·40px) + UAT 4조합.
