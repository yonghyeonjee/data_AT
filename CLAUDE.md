...
- **관심 품목은 견적서 품목표와 같은 24개 (v136 · 2026-09-21)** — "혼수/입주/이사 vs 구독 vs 일시불마다 상담 입력과 견적서가 달라져야 한다" 논의의 첫 조각. 담당자 화면 `CATS` 가
  ...
  **다음 단계(논의 중, 미구현)**: 계기(혼수·입주·이사·교체) 칸 + 예식일/입주일 → 품목 줄마다 일시불/구독·모델·예상 금액(`crm.consult_item`) → 일시불·패키지 견적서 양식(엑셀 `견적서-일반+구독` 왼쪽 혜택 계단 + 오른쪽 구독 계산표) → 카드 요율표 DB 화. 24개 목록도 언젠가 `core.item_cat` 표로 뺀다.
