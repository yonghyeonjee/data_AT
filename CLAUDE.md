...
- **매장 프로에게는 부가 기능이 안 보인다** — 재고·주문서·온라인은 원래 권한(perm)으로 숨겨져 있었고, 2026-09-13 에 `daily`(일 마감) 권한을 새로 만들어
  일 마감 탭·홈 '마감 안 한 날' 알림·미마감 배지까지 권한에 걸었다. 매장 부서(점장 포함)는 없음, 온라인사업부(고창재)·개발만 있음. 새 온라인사업부 계정은 관리자에서 perm 을 켜 준다.
  안내서·대본도 매장 프로 기준(판매 입력까지만)이다. 온라인사업부 전용 메뉴 설명은 넣지 않았다.
- **담당자 사용 안내 `/store/guide/`** — 16장: 시작 · 화면 구성 · 상황 4개(온라인 문의/매장 방문/콜백→견적서→구매/지난 상담 찾기) · 화면별 7장 · 점장이 하는 일 · 찾는 법 · 문제 시.
